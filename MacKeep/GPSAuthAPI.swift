...
class GPSAuthAPI {
  ...
  func exchangeToken(
    email: String, oauthToken: String, deviceId: String,
    completion: @escaping (Result<String, Error>) -> Void
  ) {
    log("Attempting to exchange OAuth token for master token...")
    ...
    let task = URLSession.shared.dataTask(with: request) { data, response, error in
      if let error = error {
        self.log("Error exchanging OAuth token: \(error.localizedDescription)")
      ...
      guard let data = data, let responseString = String(data: data, encoding: .utf8) else {
        ...
        self.log("Error: No data or invalid data received from token exchange endpoint.")
        completion(.failure(error))
        return
      }
      ...
      if let masterToken = responseDict["Token"] {
        self.log("Successfully retrieved master token from OAuth token.")
        ...
        let error = NSError(
          domain: "GPSAuthAPI",
          code: 1,
          userInfo: [NSLocalizedDescriptionKey: "Failed to exchange token: \(errorDetail)"]
        )
        self.log("Error: Failed to exchange OAuth token. Detail: \(errorDetail)")
        completion(.failure(error))
      }
    }
    task.resume()
